var vue = new Vue({
	...
	methods: {
		process: function(url){
			axios.post(context + 'nix/application/'+ url +'.do', {'application': this.application})
			.then(result => {
				if(result.data.responseCode == 2) {
					toastr.error(result.data.msg);
				}else if(result.data.responseCode == 1){
					toastr.success("Your request has been processed", "Success");
					window.location.href= context + 'nix/application/view.do?id='+this.application.applicationID;
				}
				else{
