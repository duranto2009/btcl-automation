var thirdTable = new Vue({
	...
	methods:{
		viewMonthlyBillSummary () {
			...
			//
            // window.open(context + 'pdf/nix-monthly-bill.do?clientId=' + clientId+'&date='+suggestedDate, "_blank" );
			window.open(context + "pdf/view/monthly-bill.do?billId="+this.app.ID + "&module=9", "_blank" );
		},
		findSummaryOfTheMonth : function() {
			...
			axios({ method: 'GET', 'url': context + 'nix/monthly-bill/monthly-bill-summary-search.do?id=' + clientId+'&date='+suggestedDate})
			.then(result => {
				
				if(result.data.responseCode == 1) {
					this.app = result.data.payload;
					if(this.app!=null){
						var items = this.app.nixMonthlyBillSummaryByItems;
						this.particulars = this.app.nixMonthlyBillSummaryByItems;
						this.adjustmentAmount = this.app.adjustmentAmount;
