var vue = new Vue({
	...
	mounted() {
		//Get Client Summary
		axios({ method: 'GET', 'url': context + 'nix/client/get-client-details.do?id=' + clientID})
		.then(result => {
			this.clientDetails = result.data.payload;
		}, error => {
		});
		//Get Contact Details
		axios({ method: 'GET', 'url': context + 'nix/client/get-contact-details-list.do?id=' + clientID})
		.then(result => {
		...
		//Get Connection List
		axios({ method: 'GET', 'url': context + 'nix/client/get-connection-list.do?id=' + clientID})
		.then(result => {
			this.connectionList = result.data.payload;
		}, error => {
		});
		//Get Long Term Contract List
		axios({ method: 'GET', 'url': context + 'nix/client/get-contract-list.do?id=' + clientID})
		.then(result => {
			this.contractList = result.data.payload;
		}, error => {
		});
		//Get TD History List
		axios({ method: 'GET', 'url': context + 'nix/td/get-client-td-history.do?id=' + clientID})
		.then(result => {
			this.tdHistoryList = result.data.payload;
		}, error => {
		});
		//Get Probable TD Date
		axios({ method: 'GET', 'url': context + 'nix/td/get-probable-td-date.do?id=' + clientID})
		.then(result => {
			this.clientTDDate = result.data.payload;
		}, error => {
		});
		//Get Balance
		axios({ method: 'GET', 'url': context + 'nix/client/get-account-list.do?id=' + clientID})
		.then(result => {
