var vue = new Vue({
    ...
    methods: {
                                ...
                                .then(result => {
                                    if (result.data.responseCode == 2) {
                                        ...
                                        if (this.picked == this.STATE_FACTORY.DEMAND_NOTE
                                            ...
                                            window.location.href = context + 'nix/dn/new.do?id=' + this.application.applicationID;
                                        } else if (
                                            this.picked == this.STATE_FACTORY.FORWARDED_LDGM_FORWARD_CDGM
                                            || this.picked == this.STATE_FACTORY.WITHOUT_LOOP_FORWARDED_LDGM_FORWARD_CDGM
        ...
        redirect: function (url) {
            var location = context + 'nix/pdf/' + url + '.do?applicationid=' + applicationID;
            if (url === 'workorder') location += '&vendorid=' + loggedInUserID;
            // http://localhost:8082/nix/pdf/view.do?type=demand-note&id=299009
            if (url === 'demandnote') location = '/nix/pdf/view.do?type=demand-note&id=' + this.application.demandNoteID;
