...
try
{
	...
	{
		double TaxAmount = apiinfo.getTaxAmount();
		...
		double AccumulatedAmount = Double.parseDouble(paid_money);
		if(TaxAmount>0.0)
		{
		AccumulatedAmount = Double.parseDouble(paid_money)-((Double.parseDouble(paid_money)*TaxAmount)/100);
		}
		
		dao.rechargeAccountByOnlinePaymentGateway(login_dto_for_cashu,AccumulatedAmount,transaction_id,desc);
		
		response_text_home_page = "Congratulation,Payment at Cashu is successful,your account will be recharged with <b>"+AccumulatedAmount+"</b> USD,including "+TaxAmount+"% Tax";
	}
	else
	{
