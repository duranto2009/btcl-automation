...
<%
	...
	String actionName = "../../GetClientForEdit.do?entityID=" + id + "&moduleID="
			+ ModuleConstants.Module_ID_LLI;
	String back = "../../GetClientForView.do?entityID=" + id + "&moduleID=" + ModuleConstants.Module_ID_LLI;
	...
	<div class="portlet-title">
		...
		<div class="actions">
			<a href="<%=back%>" class="btn btn-back-btcl"
				style="padding-left: 15px; padding-right: 15px;"> Back </a> <input
				type="button" id="printBtn" class="btn btn-default "
				style="padding-left: 15px; padding-right: 15px;" value="Print" />
			<%
				CommonActionStatusDTO commonActionStatusDTO = clientUpdateService
						.processClientUpdatePermission(clientForm.getClientDetailsDTO(), loginDTO, null);
				if (commonActionStatusDTO.getStatusCode() != CommonActionStatusDTO.ERROR_STATUS_CODE) {
			%>
			<a href="<%=actionName%>&edit" class="btn btn-edit-btcl"
				style="padding-left: 15px; padding-right: 15px;"> Edit </a>
			<%
				}
			%>
			...
			<table class="table table-bordered table-striped">
				<thead>
					<tr>
						<th class="text-center" colspan="2"><h3>
								<b><%=clientForm.getClientDetailsDTO().getLoginName()%></b>'s
								Information
							</h3></th>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<td><%=RegistrantTypeConstants.REGISTRANT_TYPE.get(ModuleConstants.Module_ID_LLI)
						.get(clientForm.getClientDetailsDTO().getRegistrantType())%></td>
					</tr>
					...
					<tr>
						...
						<td><%=new Locale("", clientForm.getClientDetailsDTO().getVpnContactDetails().get(0).getCountry())
					.getDisplayCountry()%></td>
					</tr>
					<tr>
					...
					<tr>
						...
						%>
						<td><a target="_blank"
							href="${form.registrantContactDetails.webAddress}">Visit
		...
		<div class="table-responsive">
			...
			<jsp:include page="../../common/fileListHelper.jsp" flush="true">
				<jsp:param name="entityTypeID"
					value="<%=EntityTypeConstant.LLI_CLIENT%>" />
...
<script src="${context}assets/scripts/printThis.js"
