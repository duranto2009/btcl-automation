var vue = new Vue({
    ...
    methods: {
        generateDN: function () {
            this.loading=true;
            Promise.resolve(
                axios.post(context + "lli/dn/reconnect/generate.do", {
                    dn: this.dn,
                    appId: this.applicationID,
                }).then(result => {
                    if (result.data.responseCode === 1) {
                        toastr.success('Demand Note has been generated', 'Success');
                        // window.location.href = context + "lli/pdf/view.do?type=demand-note&id=" + result.data.payload;
        ...
        getReconnectionCharge: function () {
            return floatParse(this.dn.reconnectionCharge);
        },
        ...
        getVatCalculable: function () {
            return this.getReconnectionCharge() + this.getOtherCost();
        },
    ...
    mounted() {
        this.getData(
            ...
            context + 'lli/dn/reconnect/autofill.do?appId=' + this.applicationID
        )
    },
    computed: {
        vat: function () {
