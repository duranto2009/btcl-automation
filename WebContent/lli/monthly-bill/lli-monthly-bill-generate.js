var thirdTable = new Vue({
	...
	methods:{
		findBillOfTheCurrentMonth : function() {
			...
			if(!this.isGenerated){		
				axios({ method: 'GET', 'url': context + 'lli/monthly-bill/billGenerate.do'})
				.then(result => {
					if(result.data.responseCode == 1) {
						toastr.success("Your request has been processed", "Success");
						setTimeout(function() {
							window.location.href = context + 'lli/monthly-bill/goGenerate.do';
						}, 1000);
					}
					else {
		...
		checkIsMontlyBillGenerated:function(){
			axios({ method: 'GET', 'url': context + 'lli/monthly-bill/checkIsGenerate.do'})
			.then(result => {
