var thirdTable = new Vue({
	...
	methods:{
		...
		findUsageOfTheMonth : function() {
			...
			axios({ method: 'GET', 'url': context + 'lli/monthly-usage/view.do?id=' + clientId+'&date='+suggestedDate})
			.then(result => {
				
				if(result.data.responseCode == 1) {
					this.app = result.data.payload;
					if(this.app!=null){
						this.mbpsBreakDownContent = JSON.parse(this.app.mbpsBreakDownsContent);
						this.billingRangeBreakDownContent = JSON.parse(this.app.billingRangeBreakDownsContent);
						this.longTermContractBreakDownContent = JSON.parse(this.app.longTermContractBreakDownsContent);
						this.result= true;
