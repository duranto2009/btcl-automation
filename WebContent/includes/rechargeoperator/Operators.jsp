...
<%String rechargeOperator = (String)session.getAttribute("rechargeOperator");
if(rechargeOperator == null)rechargeOperator="-1";
...
<select size="1" name="rechargeOperator">
 <option value="-1" <%=(rechargeOperator.equals("-1")?"selected=\"selected\"":"")%>>All</option> 
 <%					          
    ...
    for(int i=0;i<userList.size();i++)
    {
      ...
      {
 %>
               <option value="<%=userDTO.getUserID()%>" <%=(rechargeOperator.equals(""+userDTO.getUserID())?"selected=\"selected\"":"")%>><%=userDTO.getUsername()%></option>
 <% }} %>
