...
<div class="form-group">
	...
	<div class="col-md-8 col-sm-8 col-xs-8">
		<select class="form-control pull-right" name="paymentStatus" ><option value="" <%if (requested.equals("") ){%> selected <%}%>>All</option><option
				value="<%=BillDTO.UNPAID%>" <%if (requested.equals("0")) {%>
				selected <%}%>>Unpaid</option><option
				value="<%=BillDTO.PAID_UNVERIFIED%>" <%if (requested.equals("1")) {%>
				selected <%}%>>Paid Unapproved</option>
			<option value="<%=BillDTO.PAID_VERIFIED%>" <%if (requested.equals("2")) {%>
