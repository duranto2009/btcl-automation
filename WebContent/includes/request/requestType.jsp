	...
	<div class="col-md-8 col-sm-8 col-xs-8">
		<select class="form-control select2" size="1" name="arRequestTypeID" >
			...
			<%for(int reqType:actionTypeIDSet ){ 
				if(RequestTypeConstants.filteredOutRequestTypeIDByModuleID.get(Integer.parseInt(moduleIdStr)).contains(reqType)){continue;}
			...
			%>
			<option value="<%=reqType%>" <%=((serviceType== reqType)?"selected=\"selected\"":"")%>><%=(RequestActionStateRepository.getInstance().getActionStateDTOActionTypeID(reqType).getDescription() +" ("+rootReqTypeStr +")")%></option> 
			<%}%>
		</select>
	</div>
</div>
