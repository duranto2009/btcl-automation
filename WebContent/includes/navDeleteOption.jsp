...
<tr>
	...
	<tr>
   	<td bgcolor="#EFEFEF" style="font-family: Verdana; font-size: 8pt; font-weight: bold; padding-left: 3" >
   		<%=searchFieldInfo[searchFieldInfo.length-1][0]%>&nbsp;
   	</td>
    <td bgcolor="#EFEFEF">
          <select name="<%=searchFieldInfo[searchFieldInfo.length-1][1]%>" size="1" style="width:155px;">
           <%
          String value = (String)session.getAttribute(searchFieldInfo[searchFieldInfo.length-1][1]);
           %>
          <option value="yes" <%=(value !=null && (value.equals("yes"))? "selected='selected'":"" ) %>>Yes</option>
          <option value="no" <%=(value ==null ||(value !=null && (value.equals("no")))? "selected='selected'":"" ) %>>No</option>
          </select>
    </td>
</tr>
