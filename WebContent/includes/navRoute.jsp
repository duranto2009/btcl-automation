...
<tr>
    ...
    <td bgcolor="#EFEFEF">
    <%if(searchFieldInfo[i][1].equals("rtRouteProtocol")){%>
   		 <select size="1"  name="<%=searchFieldInfo[i][1]%>" style="width:173px">
    			<option value="-1">All</option>
				<%
				java.sql.Connection connection = databasemanager.DatabaseManager.getInstance().getConnection();
				java.sql.Statement statement = connection.createStatement();
				java.sql.ResultSet resultSet = statement.executeQuery("select * from vbProtocol");
				while(resultSet.next())
				{
 						long ptID = resultSet.getLong("ptID");
 						String ptName = resultSet.getString("ptName");
 					%>
				<option value="<%=String.valueOf(ptID)%>"><%=ptName%></option><%
				}
				resultSet.close();
				statement.close();
