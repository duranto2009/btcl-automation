...
<%
	...
	request.setAttribute("rootEntityID", id);	
	request.setAttribute("rootEntityTypeID", entityTypeID);
	...
	{
		if(clientForm.getClientDetailsDTO().getModuleID() == ModuleConstants.Module_ID_LLI && ServiceDAOFactory.getService(LLIClientTDService.class).getClientTDStatus(clientForm.getClientDetailsDTO().getClientID()).equals("TD") ){
			currentStatusStr = "<label class='label label-danger'>TD</label>";
			latestStatusName = "Temporarily Disconnected";
		}else{
		...
		<div class="tab-content">
			<%if(tabActivated.equals("1")){ %>
				<div id="tab_5_1" class="tab-pane active">
					...
					<%if(clientForm.getClientDetailsDTO().getModuleID() == ModuleConstants.Module_ID_LLI){ %>
						<h3><a href="<%=context%>lli/client/board.do?id=<%=clientForm.getClientDetailsDTO().getClientID()%>">View Board</a></h3>
					<%} %>
					</div>
