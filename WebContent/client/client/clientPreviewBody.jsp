...
<%
    ...
    long id = Long.parseLong(request.getParameter("entityID"));
    ...
    String back = "../../GetClientForView.do?entityID=" + id + "&moduleID=" + moduleID;
...
<div class="portlet box portlet-btcl">
    <div class="portlet-title">
        <div class="caption"><i class="fa fa-user"></i>Infos</div>

        <div class="actions">
            <a href="<%=back%>" class="btn btn-back-btcl" style="padding-left: 15px; padding-right: 15px; "> Back </a>
            <input type="button" id="printBtn" class="btn btn-default " style="padding-left: 15px; padding-right: 15px;"
                   value="Print"/>

            <%--any modification of client info for an active client is turned off--%>

            <%--<%if (clientUpdateService.processClientUpdatePermission(id, moduleID, loginDTO, request)){%>--%>
    ...
    <div class="portlet-body " id="printContent">
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                <thead>
                <tr>
                    <th class="text-center" colspan="2"><h3><b><%=clientForm.getClientDetailsDTO().getLoginName() %>
                    </b>'s Information</h3></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                ...
                <tr>
                    ...
                    <td><%=RegistrantTypeConstants.RegistrantTypeName.get(clientForm.getClientDetailsDTO().getRegistrantType()) %>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><%=new Locale("", clientForm.getClientDetailsDTO().getVpnContactDetails().get(0).getCountry()).getDisplayCountry()%>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>${form.registrantContactDetails.landlineNumber}</td>
                </tr>
                <%
                    if (clientForm.getClientDetailsDTO().getIdentity() != null) {
                        String[] identityArray = clientForm.getClientDetailsDTO().getIdentity().split(",");
                        for (String identity : identityArray) {
                            if (identity.trim().length() > 0 && Integer.parseInt(identity.split(":")[0]) != 0) {
                %>
                <tr>
                    <th scope="row"><%=IdentityTypeConstants.IdentityTypeName.get((Integer.parseInt(identity.split(":")[0])) % 1000)%>
                        No
                    </th>
                    <td><%=(identity.split(":").length > 1) ? identity.split(":", 2)[1] : ""%>
                    </td>
                </tr>
                <% }
                }
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a target="_blank" href="http://${form.registrantContactDetails.webAddress}">Visit Company
                            Website</a>
                        <%}%>
                    </td>
                </tr>
                <%}%>
                <%if (moduleID == ModuleConstants.Module_ID_LLI && clientForm.getClientDetailsDTO().getClientCategoryType() == 2) {%>
                <tr>
                    <th scope="row">Tarrif Category</th>
                    <td><%=new ClientTypeService().getClientCategoryByModuleIDAndClientID(moduleID, id)%>
                    </td>
                </tr>
                <%}%>
                </tbody>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <tr>
                    <th scope="row">Telephone</th>
                    <td class="text-muted">${form.billingContactDetails.landlineNumber}</td>
                    <td class="text-muted">${form.adminContactDetails.landlineNumber}</td>
                    <td class="text-muted">${form.technicalContactDetails.landlineNumber}</td>
                </tr>
                <tr>
                    <th scope="row">Fax</th>
