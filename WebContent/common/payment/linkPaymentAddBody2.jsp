...
<%@page import="vpn.bill.VpnBillDTO"%>
...
<!-- VUE Begins-->
<!-- vue -->
<script src="${context}assets/global/plugins/vue2.5.16/vue.js" type="text/javascript"></script>
<!-- vue multiselect-->
<script src="${context}assets/global/plugins/vue-multiselect/vue-multiselect.min.js" type="text/javascript"></script>
<link href="${context}assets/global/plugins/vue-multiselect/vue-multiselect.css" rel="stylesheet">
<!-- vue datepicker -->
<script src="${context}assets/global/plugins/vue-datepicker/vue-datepicker.js" type="text/javascript"></script>
<!-- vue modal -->
<script src="${context}assets/global/plugins/vue-modal/index.js" type="text/javascript"></script>
<script src="${context}assets/global/plugins/vue-modal/ssr.index.js" type="text/javascript"></script>
<script src="${context}assets/global/plugins/vue-modal/ssr.nocss.js" type="text/javascript"></script>
<link href="${context}assets/global/plugins/vue-modal/styles.css" rel="stylesheet">
<!-- vue btcl -->
<script src="${context}assets/global/scripts/btcl-vue.js" type="text/javascript"></script> 
<!-- VUE Ends -->
		...
		<html:form styleId="fileupload" styleClass="form-horizontal"
			...
			<div class="form-body">
				<div class="form-group form-md-line-input">
					<label class="col-md-3 control-label" for="form_control_1">Selected Client <span class="required" aria-required="true">*</span>
					</label>
					<div class="col-md-6">
								...
								<%if(billDTOs!=null){
									...
									EntityDTO entityDTO = null;
                                   	for(BillDTO billDTO: billDTOs){
                                   		...
                                   		else if( moduleIDTemp == ModuleConstants.Module_ID_LLI ) */
                                   			billLink = "GetPdfBillLli.do?method=getPdf&id=";
                                   	%>
								<tr>
									<td><a target="_blank"
										href="<%=context%><%=billLink %><%=billDTO.getID() %>"><%=billDTO.getID() %></a></td>
									
									<td> <%=billDTO.getActivationTimeFromStr() %></td>
									
									<td>									
										<a href="<%=context+EntityActionGenerator.getAction(moduleID, billDTO.getEntityID(), billDTO.getEntityTypeID())%>">Link Name </a>
									</td>
									<td v-model.number="btclAmount"><%=Math.round(billDTO.getTotalPayable())%></td>
									<td><%=Math.round(billDTO.getVAT()) %></td>
									<td><%=Math.round(billDTO.getNetPayable()) %></td>
						...
						<table class="table ">
							<tbody>
								<tr>
									...
									<td class="md-column"><b><%=Math.round(totalPayable)%></b></td>
									<td class="md-column"><b><%=Math.round(totalVat)%></b></td>
									<td class="md-column"><b><%=Math.round(totalPayableAmount)%></b></td>
									<td class="md-column"></td>
								</tr>
							</tbody>
						</table>
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<input class="form-control" placeholder="" name="btclAmount"
							type="text" value="<%=Math.round(totalAmount)%>" readonly>
						<div class="form-control-focus"></div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="checkbox" id="vatCheck" /> <input type="hidden"
							name="vatIncluded" id="vatCheckHidden" value="1" /> <label><span
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<input class="form-control" placeholder="" name="payableAmount"
							type="text" value="<%=Math.round(totalPayableAmount)%>" readonly>
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<input name="orderNo" type="text" class="form-control payDraftNumber" placeholder="Type here to find pay draft/cheque..." disabled/>
...
function checkPayDraftAmount(payOrderDTO){
	/*var moduleID = $("input[name=moduleID]").val();
	 ...
	 var payableAmount = Math.round(parseFloat($('input[name=payableAmount]').val()).toFixed(2));
	 var payDraftRemainingAmount = Math.round(parseFloat(payOrderDTO.payDraftRemainingAmount).toFixed(2));
...
function getTotalVat() {
    ...
    table.find('tbody  > tr').each(function (i, row) {
        var vat = parseFloat(row.cells[4].innerHTML);
        currentTotalVat += vat;
    });
    return Math.round(currentTotalVat);
}

function getTotalAmount() {
    var currentTotal = 0.0;
    table.find('tbody  > tr').each(function (i, row) {
        var amount = parseFloat(row.cells[3].innerHTML);
        currentTotal += amount;
    });
...
function getTotalPayableAmount() {
    var currentTotal = 0.0;
    table.find('tbody  > tr').each(function (i, row) {
        var amount = parseFloat(row.cells[5].innerHTML);
        currentTotal += amount;
    });
    return Math.round(currentTotal);
}
...
function getTotalItDeduction(deduction) {
    ...
    return Math.round(currentTotalDeduction);
}
...
$(document).ready(function(){
	...
	$('input[name=vatAmount]').val(Math.round(parseFloat(getTotalVat()).toFixed(2)));
	...
	$('#itDeductionCheck').change(function () {
	 ...
	 function calculateDeduction(){	    
	     var fixedTotalPayable = Math.round(getTotalPayableAmount());	     
	     var newTotalPayable = fixedTotalPayable;
	     /* $('#itDeductionCheckHidden').val(1); */
	     var deduction = Math.round($(this).val());
	     if (!isNaN(deduction) && deduction != ""){
	         var deductionAmount = Math.round(deduction);
	         newTotalPayable = fixedTotalPayable - deductionAmount;
	         LOG("netPayable: ()-) " + newTotalPayable);
	         $('input[name=payableAmount]').val(Math.round(parseFloat(newTotalPayable).toFixed(2)));
	         if(newTotalPayable < 0){
	        	 $('input[name=itDeductionAmount]').val('');
	        	 $('input[name=payableAmount]').val(Math.round(parseFloat(fixedTotalPayable).toFixed(2)));
	...
	$('#vatCheck').change(function () {
	    ...
	    //var totalPayable = Math.round(parseFloat($('input[name=payableAmount]').val()));
	    var tempDeduction = $("input[name=itDeductionAmount]").val();
	    alert('tempDeduction' + tempDeduction);
	    if(isNaN(tempDeduction) || tempDeduction === '')
	    	tempDeduction = 0;
	    
	    var  itDeductionAmount = Math.round(parseFloat(tempDeduction).toFixed(2));
	    alert(itDeductionAmount);
	    ...
	    if ( $(this).is(':checked') ) {
	        ...
	        $('input[name=vatAmount]').val(Math.round(parseFloat(totalVat).toFixed(2)));
	        
	        var netPayable = totalPayable - itDeductionAmount;
	        
	        $('input[name=payableAmount]').val(Math.round(parseFloat(netPayable).toFixed(2)));
	        $(".vat").css("display","none");
	...
	$("#action").click(function( event ) {
		  ...
		  if(payDraftID == ""){
			  ...
			  var  payableAmount = Math.round(parseFloat($("input[name=payableAmount]").val()).toFixed(2));
			  var availableAmount = Math.round(parseFloat($("input[name=paidAmount]").val()).toFixed(2));
...
<script>
	 ...
	 var app = new Vue({
	 	...
	 	total() {
         return this.btclAmount.reduce(getSum);
       	},
       	getSum(total, num)
       	{
       		return total + num;
