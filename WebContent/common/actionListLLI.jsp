...
/* 					int entityTypeIDActionScript = (Integer)(request.getAttribute("entityTypeID"));
					...
					for (StateActionDTO stateActionDTO1 : stateActionDTOs) {
						...
						else
						{
							...
							if( row.getRequestTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_REQUEST_ADVICE_NOTE ){
	...
	<%-- 							<h4 class="text-center bold "> <%=stateName %></h4>
								<div><%=stateActionDTO1.getCommonRequestDTO().getDescription()%></div> --%>
									...
									<table class="table table-bordered table-striped">
										<tbody>
											...
											<th scope="row">Root Request</th>
												<th>
													<a href="<%=context%><%=new LliLinkService().generateRequestPreviewURL(row.getRootReqID()==null?row.getReqID():row.getRootReqID(), row.getRequestTypeID())%>" target="_blank"><%=rootReqTypeStr%></a>
												</th>
											</tr>
											<tr>																								
												...
												<a target="_blank" href="<%=context%>GetPdfBillLli.do?method=getPdf&reqID=<%=row.getReqID()%>">VIEW</a>
												<%}%>
	                            ...
	                            if(samePerson && revertable){
	                            	...
	                            	sourceCommonRequestDTO.setRequestTypeID(LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_CANCEL_REQUEST);
	...
	//							actionStateDTO.setStateID(stateActionDTO.getStateID());
								...
								String [][] searchFieldInfo = LliRequestTypeConstants.requestTypToSearchCriteria.get(actionStateDTO.getActionTypeID());
								
								if( actionTypeID == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_GENERATE_DEMAND_NOTE ||
										actionTypeID == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_GENERATE_MIGRATION_DEMAND_NOTE ||
										actionTypeID == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_GENERATE_MIGRATION_MRC ||
										actionTypeID == LliRequestTypeConstants.REQUEST_UPGRADE.SYSTEM_GENERATE_DEMAND_NOTE){
				...
				<%
						...
						actionLogger.debug("actionStateDTO.getType() " +actionStateDTO.getActionTypeID());
					 	...
					 	SqlGenerator.populateObjectFromOtherObject(commonRequestDTO, CommonRequestDTO.class, stateActionDTO1.getCommonRequestDTO(), CommonRequestDTO.class);	
						...
						else if(LliRequestTypeConstants.remainderSetOfResponseBack.contains(requestTypeRemainder))
						{
						...
						if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.GET_PREPARED_TO_GENERATE_DEMAND_NOTE)
						{
						...
						%>
					<%if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.UPDATE_APPLICATION){ %>					
						<div class="portlet" id="link_action_<%=stateActionDTO1.getStateID()%>_<%=actionStateDTO.getActionTypeID() %>" style="display: none;">
							<br>
							<a class="btn btn-sm green-jungle  uppercase" href="<%=searchFieldInfo[0][0]+"entityID="+request.getAttribute("entityID")+"&entityTypeID="+EntityTypeConstant.LLI_LINK+"&getMode=edit"%>"><%=searchFieldInfo[0][1] %></a>
						</div>
					<%
					continue;
					
					}else if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.UPDATE_APPLICATION
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_RESPONSE_WITH_INTERNAL_FR
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_POP_CHANGE.SYSTEM_RESPONSE_WITH_INTERNAL_FR
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_UPGRADE.SYSTEM_RESPONSE_WITH_INTERNAL_FR
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_DOWNGRADE.SYSTEM_RESPONSE_WITH_INTERNAL_FR
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_IPADDRESS.SYSTEM_RESPONSE_WITH_INTERNAL_FR
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.GET_PREPARED_TO_GENERATE_DEMAND_NOTE
						|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_POP_CHANGE.GET_PREPARED_TO_GENERATE_DEMAND_NOTE
						){
						
						String url=searchFieldInfo[0][0]+"entityID="+request.getAttribute("entityID")+"&entityTypeID="+EntityTypeConstant.LLI_LINK;
					...
					}else if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_REQUEST_WITH_INTERNAL_FR){							
					%>
					<%}else if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_RESPONSE_EXTERNAL_FR_OF_NEAR_END
					|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_RESPONSE_EXACT_EXTERNAL_FR_OF_NEAR_END){
					
					    //commonRequestDTO.setRequestToAccountID(stateActionDTO1.getCommonRequestDTO().getRequestByAccountID());						
						%>						
					<%}else if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_RESPONSE_EXTERNAL_FR_OF_FAR_END
					|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_RESPONSE_EXACT_EXTERNAL_FR_OF_FAR_END){	
						
						//commonRequestDTO.setRequestToAccountID(stateActionDTO1.getCommonRequestDTO().getRequestByAccountID());						
						%>
					
					<%}
					else if( actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_REQUEST_ADVICE_NOTE 
							|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_DISABLE_LINK.SYSTEM_REQUEST_ADVICE_NOTE_FOR_DISABLE
							|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_DISABLE_LINK.SYSTEM_REQUEST_ADVICE_NOTE_FOR_ENABLE
							|| actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_LINK_CLOSE.SYSTEM_REQUEST_ADVICE_NOTE
							){																	
					}
					else if( actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_NEW_LINK.SYSTEM_INFORM_TESTING_DONE ){						
						commonRequestDTO.setRequestToAccountID(stateActionDTO1.getCommonRequestDTO().getRequestByAccountID());		
					}else if(actionStateDTO.getActionTypeID() == LliRequestTypeConstants.REQUEST_UPGRADE.SYSTEM_REQUEST_WITH_INTERNAL_FR){												
