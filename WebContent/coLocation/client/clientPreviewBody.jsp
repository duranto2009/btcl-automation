...
<%
	...
	String actionName = "../../GetClientForEdit.do?entityID=" + id+"&moduleID="+ModuleConstants.Module_ID_COLOCATION;
	String back = "../../GetClientForView.do?entityID=" + id+"&moduleID="+ModuleConstants.Module_ID_COLOCATION;
	...
	<div class="portlet-title">
		...
		<div class="actions">
			...
			<input type="button" id="printBtn" class="btn btn-default " style="padding-left: 15px; padding-right: 15px;" value="Print" />  
		 	<% if (new ClientUpdateService().isUserPermittedToUpdateClient(Long.parseLong(request.getParameter("entityID")), ModuleConstants.Module_ID_COLOCATION*100+51, (LoginDTO)request.getSession(true).getAttribute(SessionConstants.USER_LOGIN))
					&& new ClientService().getClientIsActive(Long.parseLong(request.getParameter("entityID")), ModuleConstants.Module_ID_COLOCATION)){%>
				...
				<thead>
					...
					%>
						...
						<tr>
							...
							<td><%=RegistrantTypeConstants.REGISTRANT_TYPE.get(ModuleConstants.Module_ID_COLOCATION).get(clientForm.getClientDetailsDTO().getRegistrantType()) %></td>
						</tr>
								...
								<%
									...
									for (Integer availableItem : regMap.keySet()) {
										if(registrantCategory == availableItem)){ %>
											<span class="badge badge-success"> <%=regMap.get(availableItem)%> </span>
									<%}
								}%>
							</td>
						</tr>
