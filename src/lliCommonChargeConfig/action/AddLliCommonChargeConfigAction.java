package lliCommonChargeConfig.action;
...
import lliCommonChargeConfig.LliCommonChargeConfigService;
import lliCommonChargeConfig.form.lliCommonChargeConfigForm;

public class AddLliCommonChargeConfigAction extends Action {
	...
	public ActionForward execute(ActionMapping p_mapping, ActionForm p_form, HttpServletRequest p_request,
		...
		lliCommonChargeConfigForm form = (lliCommonChargeConfigForm) p_form;
		
		CommonChargeDTO dto = new CommonChargeDTO();

		LliCommonChargeConfigService service = new LliCommonChargeConfigService();
		
		try {
			dto.setId(DatabaseManager.getInstance().getNextSequenceId("at_lli_common_charge_config"));
			...
			target = "failure";
			logger.fatal("Error inside Add lli common charge config action "+e);
		}
