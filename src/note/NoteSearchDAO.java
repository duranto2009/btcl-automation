...
public class NoteSearchDAO {
	
Logger logger = Logger.getLogger(getClass());
	
	private String generateAdditionalCondition() throws Exception {
		String requestTypeIdField = getColumnName(CommonRequestDTO.class,"requestTypeID");
		String condition = "(";
		int i = 0;
		for(Map.Entry<Integer, String> entry : NoteTypeConstants.mapOfNoteTypeToNoteTypeIdExtension.entrySet()){
			condition += i==0?"":" OR ";
			condition +=  requestTypeIdField + " LIKE '%"+entry.getKey() +"' ";
			i++;
		}
	...
	public Collection getIDsWithSearchCriteria(int moduleID,Hashtable<String, String> criteriaMap, LoginDTO loginDTO, DatabaseConnection databaseConnection)  throws Exception {
		...
		String fixedCondition  = " floor(("+ getColumnName(CommonRequestDTO.class,"entityTypeID")+"/ "+EntityTypeConstant.MULTIPLIER2+") + 0.001) = "+moduleID + " AND "
				+generateAdditionalCondition();
		...
		fixedCondition += filterOutRequestTypeCondition();
	...
	private String filterOutRequestTypeCondition() throws Exception {
		String requestTypeFilteringCondition = "";
		if(RequestTypeConstants.filteredOutRequestTypeIDByModuleID.size()>0) {
			requestTypeFilteringCondition = " and " + getColumnName(CommonRequestDTO.class, "requestTypeID") + " not in ";
			requestTypeFilteringCondition += StringUtils.getCommaSeparatedString(RequestTypeConstants.filteredOutRequestTypeIDByModuleID.values());
		}
