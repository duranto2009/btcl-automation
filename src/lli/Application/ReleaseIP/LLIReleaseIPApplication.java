...
@TableName("at_lli_application_release_ip")
@ForeignKeyName("parentApplicationID")
public class LLIReleaseIPApplication extends LLIApplication implements SingleConnectionApplication{
	@PrimaryKey
	...
	public void insertApplication() throws Exception {
		this.setApplicationType(LLIConnectionConstants.RELEASE_IP);
	...
	public void setImmutablePropertyWhileProcessing(LLIApplication lastApplicationSnapshot) throws Exception {
		if(!(lastApplicationSnapshot instanceof LLIReleaseIPApplication)) {
		...
		LLIReleaseIPApplication lliReleaseIPApplication = (LLIReleaseIPApplication) lastApplicationSnapshot;
		String content = this.getContent();
		ModifiedSqlGenerator.populateObjectFromOtherObject(lliReleaseIPApplication, this, LLIReleaseIPApplication.class);
		setContent(content);
	}
	@SuppressWarnings("static-access")
	@Override
